###
# Project - regional flex dashboard
# Title - region maps
...
eng_nuts1 <- st_read(here("Dashboard fun", "data",
...
eng_nuts3 <- st_read(here("Dashboard fun", "data",
...
itl1_map_highlight <- function(region){
  if(region == "UKIJ"){
    itl1_select <- st_read(here("Dashboard fun", "data",
                                  "NUTS1 - shape file", "NUTS_Level_1_(January_2018)_Boundaries.dbf")) %>%
...
itl3_map_highlight <- function(region){
  if(region == "UKIJ"){
    itl3_select <- st_read(here("Dashboard fun", "data",
                                "NUTS3 - shape file", "NUTS_Level_3_(January_2018)_Boundaries.dbf")) %>% 
...
# # colour gradient map ----------------------------------------------------------
# itl1_region <- st_read(here("Dashboard fun", "data",
#                             "NUTS3 - shape file", "NUTS_Level_3_(January_2018)_Boundaries.dbf")) %>% 
#   mutate(nuts1 = substr(nuts318cd, 1, 3)) %>% 
#   filter(!nuts1 %in% c("UKL", "UKM", "UKN")) %>% 
