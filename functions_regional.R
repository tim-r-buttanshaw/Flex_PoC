...
itl2_lookup_names <- read_csv(here("Dashboard fun", "data", "ITL2 codes from ONS website.csv")) %>% 
  select(-ObjectId) %>%
  clean_names %>% 
...
top3_finder <- function(item_code){
  ...
  top3_itl1_across <- everything_cleaned %>% 
    ...
    group_by(nuts1_code, year) %>% # group by itl1 and year so the correct values are summed 
